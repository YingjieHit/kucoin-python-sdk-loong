...
class MarketDataAsync(KucoinFuturesBaseRestApiAsync):
    ...
    async def get_last_n_kline(self, symbol, kline_type, n, end_at=None):
        if end_at is None:
            end_at = int(datetime.now().timestamp())
        start_at = end_at - utils.calc_second_by_freq(kline_type, n)

        return await self.get_kline(symbol, kline_type, startAt=end_at, endAt=start_at)
    ...
    async def get_server_timestamp(self):
        ...
        """
        raise NotImplementedError("需要实现get_server_timestamp")
    ...
    async def get_server_status(self):
        ...
        """
        raise NotImplementedError("需要实现get_server_status")
